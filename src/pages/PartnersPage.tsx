...
const PartnersPage = () => {
  ...
  const { data: rawItems = [], isLoading } = useGetList<Partner>("partners", "/partners");
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: "", type: "OTHER", sortOrder: 0, isActive: true, logoUrl: "" },
  });

  const openCreate = () => { setEditItem(null); form.reset({ name: "", type: "OTHER", sortOrder: 0, isActive: true, logoUrl: "" }); setFormOpen(true); };
  const openEdit = (item: Partner) => {
    setEditItem(item);
    form.reset({ name: item.name, websiteUrl: item.websiteUrl || "", type: item.type, sortOrder: item.sortOrder, isActive: item.isActive, logoUrl: item.logoUrl || "" });
    setFormOpen(true);
  };

  const onSubmit = (values: FormValues) => {
    if (editItem) {
      updateMutation.mutate({ id: editItem.id, body: values }, { onSuccess: () => { toast.success("Hamkor yangilandi"); setFormOpen(false); } });
    } else {
      createMutation.mutate(values, { onSuccess: () => { toast.success("Hamkor yaratildi"); setFormOpen(false); } });
    }
  };

  return (
    <div className="space-y-6">
      <PageHeader title="Hamkorlar" description="Hamkorlarni boshqaring" icon={Handshake} actionLabel="Yangi hamkor" onAction={openCreate} />
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          <SelectTrigger className="sm:w-48"><SelectValue placeholder="Turi" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="ALL">Barchasi</SelectItem>
            {(Object.keys(partnerTypeLabels) as PartnerType[]).map((t) => (
              <SelectItem key={t} value={t}>{partnerTypeLabels[t]}</SelectItem>
            ))}
          </SelectContent>
        </Select>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <SortableHead field="name" sortBy={table.sortBy as string} sortOrder={table.sortOrder} onSort={(f) => table.toggleSort(f as keyof Partner)}>Nomi</SortableHead>
                <TableHead>Turi</TableHead>
                <TableHead>Veb-sayt</TableHead>
                <SortableHead field="sortOrder" sortBy={table.sortBy as string} sortOrder={table.sortOrder} onSort={(f) => table.toggleSort(f as keyof Partner)}>Tartib</SortableHead>
            ...
            <TableBody>
              {table.filtered.map((item) => (
                <TableRow key={item.id} data-state={table.selectedIds.has(item.id) ? "selected" : undefined}>
                  ...
                  <TableCell><StatusBadge variant="muted">{partnerTypeLabels[item.type] || item.type}</StatusBadge></TableCell>
                  <TableCell className="text-muted-foreground">{item.websiteUrl || "—"}</TableCell>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editItem ? "Hamkorni tahrirlash" : "Yangi hamkor"}</DialogTitle></DialogHeader>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <div><Label>Nomi *</Label><Input {...form.register("name")} /></div>
            <div><Label>Veb-sayt</Label><Input {...form.register("websiteUrl")} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Turi *</Label>
                <Select value={form.watch("type")} onValueChange={(v) => form.setValue("type", v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{(Object.keys(partnerTypeLabels) as PartnerType[]).map((t) => (<SelectItem key={t} value={t}>{partnerTypeLabels[t]}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div><Label>Tartib raqami</Label><Input type="number" {...form.register("sortOrder")} /></div>
            </div>
            <div><Label>Logo</Label><FileUpload value={form.watch("logoUrl")} onChange={(url) => form.setValue("logoUrl", url)} /></div>
      ...
      <ConfirmDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)} onConfirm={() => {
        if (deleteId) deleteMutation.mutate(deleteId, { onSuccess: () => { toast.success("Hamkor o'chirildi"); setDeleteId(null); } });
